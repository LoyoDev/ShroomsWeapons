...
{
	public class DarkPrismBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / DarkPrismHoldout.NumBeams;
		...
		{
			DisplayName.SetDefault("Dark Prism Beam");
		}
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<DarkPrismBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<DarkPrismHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / DarkPrismHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > DarkPrismHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - DarkPrismHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * DarkPrismHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / DarkPrismHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
